...
| UI visual contract | unchanged in this PR | UI再設計・画像生成・DOM変更は別freeze ticket |
...
本PRはUIを生成・変更しない。現行consoleのvisual contractは維持する。次のUI freezeでは、
既存FishVJと同じ見た目を成功条件にせず、次を別画面契約として作る。

- source browser: stack/worldを区別。
- world capability/target inspector: world/system/group/entity scopeを明示。
- performance map editor: 1 control→1 event、固定mixerはlock表示。
- surface topology/calibration view。
- preview/programの明確な分離。

画像だけで操作意味を決めず、Performance Map schemaとproof acceptanceを先に正とする。
