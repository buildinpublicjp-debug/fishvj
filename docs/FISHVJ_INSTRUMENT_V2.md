...
| UI visual contract | extended | 現行FishVJ consoleは不変。World operator console v2を別screenとして画像freeze |
...
[FishVJ World Operator UI Visual Contract v2](./design/FISHVJ_WORLD_UI_VISUAL_CONTRACT.md)と
正典画像`fishvj-world-operator-master-v2.png`を、演者向けWorld consoleの人間側referenceとしてfreezeする。
[FishVJ World Auxiliary Screens Visual Contract v1](./design/FISHVJ_WORLD_AUX_SCREENS_VISUAL_CONTRACT.md)と
Source Browser、Performance Map Editor、Surface Topology / Calibrationの正典画像3枚も同じschema-first原則で
freezeする。
...
- bounded deckの`PLAY / CUE / HOT CUE`とread-only manifest loop範囲。performer-selectable beat loopは含めない。
...
- sourceとmapをatomic `1 event`でloadするSource Browser。
- fixed mixerとprogram scopeをlockし、frozen defaultのcopyを新ID/hashで保存するPerformance Map Editor。
- physical program 1面、offscreen fixture、camera 1件、4点homographyを示すSurface Topology / Calibration。
