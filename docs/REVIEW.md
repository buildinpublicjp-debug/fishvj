...
| R-010 | open | P1 | 3モードへ2.5Dカメラパスを追加する | adopt（範囲限定） |
| R-011 | open | P2 | ヒーロー魚GLBを1体seed同梱する | adopt（事前完了条件） |
...
- priority: P1
- 対象候補: `SPEC.md`「3モード」「デモ設計」、`TECH.md`「魚群」「3hタイムライン」
...
- source: 外部レビュー（Claude）
- priority: P2
- 対象候補: `SPEC.md`「レイヤースタック」「デモ設計」「事前準備チェックリスト」、`TECH.md`「描画詳細」「3hタイムライン」「8Kロードマップ」
