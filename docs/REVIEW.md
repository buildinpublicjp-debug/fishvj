...
| R-020 | open | P1 | 魚種図鑑から素材を選び群れへ投入する操作が未定義 | adopt（seed図鑑） |
| R-021 | open | P1 | 魚種が増えても泳ぎ方が一種類では個体差が伝わらない | adopt（4泳動型） |
| R-022 | open | P1 | 無限落下・再帰トンネルを明示的に呼び出す演出がない | adopt（既存機能のプリセット） |
| R-023 | open | P0 | 生成した完成画像と実時間レンダーの到達点が混同されている | adopt（再構成契約） |
| R-024 | open | P1 | 現行PlaneGeometryでは魚体のうねりに必要な頂点数と素材向きが未定義 | adopt（分割Plane） |
