...
| ID | status | 優先度 | 要旨 | 推奨 |
|---|---|---:|---|---|
| R-001 | open | P0 | 現行画像モデルと透過魚スプライト要件が両立しない | adopt |
| R-002 | open | P0 | 生成APIの待ち時間・利用条件・応答サイズがデモ前提として未検証 | adopt |
| R-003 | open | P0 | 3h計画に出荷バッファがなく、journey機能も工数計上されていない | adopt |
| R-004 | open | P1 | `high`帯域上限とハイハット検出帯域が矛盾している | adopt |
| R-005 | open | P1 | ボーカル検出の表現が実装予定のヒューリスティックより強い | adopt |
| R-006 | open | P0 | 本線MP3に決定論的なBPM同期フォールバックがない | adopt |
| R-007 | open | P0 | 90秒本番中のマイク切替が許可・入力・フィードバック事故を招く | adopt |
| R-008 | open | P0 | 1080p出力に4K canvas、最大1万魚を初期値とする前提が過大 | adopt |
| R-009 | open | P2 | WebGPU移行時にカスタムGLSL/ポストFXが大半そのまま生存する保証はない | adopt |
...
- 推奨: `adopt`。3h版は透過魚を同梱seedへ固定し、ライブ生成は背景・曼荼羅リングなど不透明でも成立する素材を `gpt-image-2` で生成する。透過魚のライブ生成はハッカソン後の背景除去パイプライン検証項目に分離する。
- 反映先: 未反映（判断待ち）
- 棄却理由: —
- 根拠:
...
- 推奨: `adopt`。事前チェックに「本番アカウントで1枚生成成功」「利用モデル・IPM・Organization Verification確認」「本番経路で応答サイズ計測」を追加する。デモは生成完了を必須にせず、1リクエストずつ・低品質圧縮形式・タイムアウト表示・seed継続を契約にする。
- 反映先: 未反映（判断待ち）
- 棄却理由: —
- 根拠:
  - https://developers.openai.com/api/docs/guides/image-generation
...
- 推奨: `adopt`。2:20を機能凍結、残り40分をデプロイ・README・動画・提出に固定する。journeyはコア完成・本番URL・録画成功後だけ着手するstretch goalにし、未実装でも3モードでコンセプトを説明可能にする。
- 反映先: 未反映（判断待ち）
- 棄却理由: —
...
- 推奨: `adopt`。`high` を2k〜12kHzへ統一するか、ハイハットだけFFT binの6k〜12kHzを直接集計すると明記する。3hでは前者を推奨。
- 反映先: 未反映（判断待ち）
- 棄却理由: —
...
- 推奨: `adopt`。信号名と説明を「vocal/lead proxy（中域持続エネルギー）」へ正確化し、Suno曲のチャント区間で成立する閾値を事前調整する。汎用の歌声分離は3h版の対象外と明記する。
- 反映先: 未反映（判断待ち）
- 棄却理由: —
...
- 推奨: `adopt`。同梱曲には既知BPM（予定138）をメタデータとして持たせ、推定が安定するまで、または信頼度低下時は既知値で位相駆動する。タップテンポは手動復旧経路として残す。
- 反映先: 未反映（判断待ち）
- 棄却理由: —
...
- 推奨: `adopt`。開始前に許可と入力確認を済ませ、UIにgreen状態が出た場合だけ切り替える。未確認または不安定なら90秒本編では切り替えず、質疑用の保険へ回す。
- 反映先: 未反映（判断待ち）
- 棄却理由: —
...
- 推奨: `adopt`。当日既定値を出力実解像度、devicePixelRatio上限1、魚2,000体、Bloom半解像度にする。60fps実測後だけ段階的に増やし、4K/8Kはロードマップとする。
- 反映先: 未反映（判断待ち）
- 棄却理由: —
...
- 推奨: `adopt`。生存範囲を「アプリ状態・音解析・シーン設計」に限定し、「カスタムシェーダーとポストFXは移植検証対象」と明記する。ハッカソン実装方針は変更しない。
- 反映先: 未反映（判断待ち）
- 棄却理由: —
